...
class CommandRunner(ABC):
    ...
    def run(self, project: Project) -> None:
        """Execute command"""
        self.log_info(f"{project.config.models_directory=}")
        self._run(project)
...
class CleanupCommandRunner(CommandRunner):
    ...
    def _run(self, project: Project) -> None:
        ...
        files_removed = self._cleanup(project, project.config.models_directory)
        self.log_info(f"{files_removed=}")

    def _cleanup(self, project: Project, folder: Path) -> int:
        """
        ...
        files = tuple(project.config.search_by_masks_recursive(folder, self.masks))
...
class DisplayModelCommandRunner(CommandRunner):
    ...
    def _run(self, project: Project) -> None:
        try:
            part = project.get_part_model(self.identifier)
            ...
            try:
                unit = project.get_assembly_unit_model(self.identifier)
    ...
    def _display_model(self, model: Model) -> None:
        self.log_info("")
        self.log_info(f"{model.__class__.__name__} '{model.content_directory}':")

        renders_count = len(model.renders)
        if renders_count == 0:
            return

        self.push()
        self.log_info(f"Renders: {renders_count}")

        self.push()
        for i in model.renders:
